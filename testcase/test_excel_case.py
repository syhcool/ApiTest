...
#1、初始化信息
#1）.初始化测试用例文件
case_file = os.path.join(Conf.get_data_path(),ConfigYaml().get_excel_case_file())
#2）.测试用例sheet名称
sheet_name = ConfigYaml().get_excel_case_file_sheet()
#3）.获取运行测试用例列表
...
#1）.初始化信息，url,data
    ...
    def test_run(self,case):
        ...
        assert_util.asser_code(int(res["code"]),int(code))
        #返回结果内容
        assert_util.asser_in_body(str(res["body"]),str(expect_result))
        #数据库结果断言
